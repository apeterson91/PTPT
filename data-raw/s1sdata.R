...
## data frame gets bigger because some trips are duplicated for 
## the same origin-destination pairs (efficiency)
gtdf <- gtdf %>% 
    left_join(odf %>% rename(EDistance = Distance)) %>% 
    left_join(pcl_samples %>% 
                  st_drop_geometry() %>% 
                  select(origin_ix,hood,subject_hood_id))
