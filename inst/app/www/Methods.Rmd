...
tibble(`Organization` = c("WPRDC","WPRDC","WPRDC",
    ...
    gt(caption = "Table 1") %>% 
    tab_header("Data Sources used in the PTPT")
```
### Big Picture
...
and we plan to utilize methods from the post-stratification literature 
...
travel - to complete a within-city (or potentially within-county) trip, $\pi$. 
Estimates for this value are typically distinguished by the type of trip, e.g. 
commute, which we denote $\pi_c$. According to US census data compiled by the 
American League of Bicyclists, $\pi_c \approx 2$% [@mcleod_2016; @mcleod_2017]. 
However, these figures are from 2017 and the coronavirus pandemic have 
undoubtedly altered these numbers, as working from home is now a 
